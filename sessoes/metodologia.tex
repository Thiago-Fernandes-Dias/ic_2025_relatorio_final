...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{diagrams/deepprint-arch.png}
    \caption{Arquitetura do \textit{DeepPrint}}
...
No processo de verificação entre uma digital de consulta (\textit{query}) e uma de referência (\textit{gallery}), os descritores tridimensionais e suas máscaras são achatados em vetores unidimensionais para a realização das comparações matriciais5. A pontuação de similaridade $M_i(q, g)$ para cada par de imagens é calculada pela fórmula do cosseno normalizado $M_i(q, g) = \frac{{f_q'}^T \cdot f_g'}{\|f_q \odot S_g\|_F \cdot \|f_g \odot S_q\|_F}$6. No numerador, a multiplicação matricial do vetor transposto de consulta pelo vetor de referência (${f_q'}^T \cdot f_g'$) realiza o produto escalar das características apenas nas regiões onde ambas as impressões possuem primeiro plano válido, visto que o fundo de cada imagem já foi previamente anulado pela sua respectiva máscara46.No denominador, o produto de Hadamard cruzado ($f_q \odot S_g$ e $f_g \odot S_q$) filtra o descritor de uma imagem utilizando a máscara da outra, isolando estritamente a área de intersecção espacial compartilhada entre elas. Em seguida, a norma de Frobenius ($\|\cdot\|_F$) calcula a magnitude desse tensor resultante na região sobreposta, atuando como o fator de normalização da similaridade de cosseno para evitar distorções causadas por variações no tamanho da área de contato.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{diagrams/flare-arch.png}
    \caption{Arquitetura do FLARE}
    \label{fig:flare-arch}
\end{figure}
